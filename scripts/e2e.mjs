...
for (const variant of withoutPackagesVariants) {
    ...
    assertEqual(['netsuite-api.config.json', 'scripts/addJobs.mjs', 'client/src/hooks/apiErrors/useApiErrors.ts', 'how-to-use/controllers', '.claude/rules/controllers.md'].map(variantFileExists), Array(5).fill(variant.netsuiteApi), `${label}: what netsuite-api brings is there only with it`);
