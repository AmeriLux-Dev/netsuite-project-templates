...
- **Records and fields:** declared on the models in `common/model/`; each model names its record type and every field id it reads or writes. The generated entity types in `common/types/models.gen.ts` are what each controller's request and response shapes (declared in its controller file) pick from.
