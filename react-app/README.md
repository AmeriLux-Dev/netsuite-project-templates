...
Suitelet-hosted React application for NetSuite. How it is laid out and how to extend it is in [how-to-use/](./how-to-use/folder-structure.md): the folder structure{{#if codeGeneration}}, and {{#if netsuiteApi}}worked examples{{/if}}{{#unless netsuiteApi}}a worked example{{/unless}} of {{#if netsuiteRepository}}a repository{{/if}}{{#if bothNetsuitePackages}}, {{/if}}{{#if netsuiteApi}}a controller and a job{{/if}}{{/if}}; this file is the record of what the application is for, who owns it, and why it looks the way it does.
...
- **Records and fields:** declared on the models in `api/src/models/`; each model names its record type and every field id it reads or writes. The generated entity types in `api/src/types/models.gen.ts` are what the services' types{{#if netsuiteApi}} and each controller's request and response shapes (declared in its controller file){{/if}} pick from.
{{/if}}
- {{#if netsuiteApi}}**Scripts:** each controller declares its own script and deployment ids in its `defineRestlet` or `defineSuitelet` call. {{/if}}**{{#if codeGeneration}}Other ids{{/if}}{{#unless codeGeneration}}Ids{{/unless}}:** `netsuite.ts` (the application's names, and {{#if codeGeneration}}any id no {{#if netsuiteRepository}}model{{/if}}{{#if bothNetsuitePackages}} or {{/if}}{{#if netsuiteApi}}controller{{/if}} owns{{/if}}{{#unless codeGeneration}}its NetSuite ids{{/unless}}: {{#unless netsuiteApi}}script and deployment ids, {{/unless}}{{#unless netsuiteRepository}}records, fields, {{/unless}}script parameters, saved searches, list values).
- **Packages:** {{#if netsuiteApi}}`@amerilux/netsuite-api` (endpoints, the generated browser client), {{/if}}{{#if netsuiteRepository}}`@amerilux/netsuite-repository` (data access), {{/if}}`@amerilux/netsuite-wrapper` (instrumented `N/*` calls).
...
- **Logs:** every deployed script writes to its own NetSuite script log (Customization › Scripting › Script Deployments). Log titles are constant phrases{{#if netsuiteApi}} (`endpoint completed`, `endpoint rejected`, `endpoint failed`); the controller, endpoint, method, status and ids are in the details{{/if}}{{#unless netsuiteApi}}; the ids are in the details{{/unless}}.
