...
- `common/` is shared by api and client and never imports `N/*`.
  ...
  - `common/types/api.ts` holds the response envelope and the endpoint types both sides share (`Endpoints`, `EndpointRequest`, `EndpointResponse`). Nothing controller-specific lives in `common/`.
...
- `api/src/_lib/`: transport plumbing (`defineEndpoints`, `defineRestlet`, `defineSuitelet`, `ApiError`) plus `suiteletClient.ts`, which calls another Suitelet controller of this application server-side: `createSuiteletClient<UserRolesEndpoints>(scripts.userRoles)`. Only a repository builds one.
...
- `client/src/routes/`: file-based routing (TanStack Router, hash history); `routeTree.gen.ts` is produced by the Vite plugin. Pages live under `client/src/pages/`, hooks under `client/src/hooks/`, one API module per controller under `client/src/api/`, each `createApiClient<<Name>Endpoints>(scripts.<name>)` with the type imported from `api/controllers/<name>Controller` as a type only: one typed function per endpoint (`userApi.roles()`, `ordersApi.byId({ id })`, options such as the abort signal second), the URL picked from the script's `kind`. The client never imports api code, only that type. A page calls a hook; only a hook calls `client/src/api`.
