...
- `client/src/routes/`: file-based routing (TanStack Router, hash history); `routeTree.gen.ts` is produced by the Vite plugin. Pages live under `client/src/pages/`, hooks under `client/src/hooks/`. `client/src/api/index.gen.ts` is generated from the controllers: every request and response type, every endpoint type, and one typed client per controller without `browser: false` (`userApi.roles()`, `ordersApi.byId({ id })`, options such as the abort signal second), the URL picked from the script's `kind`; `client/src/api/models.gen.ts` is the copy of the entity types it imports. A hook imports `@/api/index.gen`; a page or component that needs `app` imports `@/app.gen`. The client never imports from `api/` or `netsuite.ts`. A page calls a hook; only a hook calls the generated client module.
