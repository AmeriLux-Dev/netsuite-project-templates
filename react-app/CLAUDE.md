...
- `api/src/repositories/`: data access. Each function creates the context it needs with `createAppContext()` from `generated/context.gen` (`{ tracking: false }` for reads), composes specifications over its record sets, and finishes its own write before returning. A flow that reads and writes several records under one change tracker is a single repository function. `generated/` is produced by `npm run generate` and holds the models' types, fields and the context.
