...
Use the `add-controller` skill (`.claude/skills/add-controller/SKILL.md`); it lists every file and the exact shape of each, with the customers controller as the reference. Never hand-write a script id anywhere but `common/netsuite.ts`. `npm run lint` fails until all the pieces are present and agree.
...
- `common/` is shared by api and client and never imports `N/*`.
  ...
  - `common/dto/<controller>.ts` holds the request and response shapes of a controller's endpoints: the wire, not the record. A DTO picks from the generated entity type (`Pick<Customer, 'id' | 'companyName'>`) so it follows the model, and carries nothing the client does not need.
