...
| `npm run lint` | ESLint over the repository, then `scripts/checkStructure.mjs`: every script's pieces agree (ids, SDF object, controller folder, endpoints file, client module). |
...
- `api/src/controllers/<name>/`: one folder per controller, two files. `endpoints.ts` declares the request and response shapes (DTOs: an entity type from `common/types/models.gen.ts`, a `Pick` of one, or a composition of several; the wire, not the record) and then `<name>Endpoints = defineEndpoints({ ... })`: one transport-agnostic function per endpoint (`list`, `byId`, `create`), its parameter the request and its return value the response, plus `export type <Name>Endpoints = typeof <name>Endpoints`. The handler signatures are the contract; there is no other declaration. Every call is a POST whose JSON body carries the request and an `endpoint` property naming the endpoint. `<name>Controller.ts` is the only transport-specific file and serves the endpoints through `defineRestlet` (exports `post`) or `defineSuitelet` (exports `onRequest`) from `api/src/lib/`, which supply the envelope, error mapping and timing. A file becomes a deployed script only when its leading JSDoc carries `@NScriptType`; everything else under `api/src/` is bundled into the scripts that import it. Endpoints stay thin: call a service, return its result. Switching transport touches the controller file, its SDF object and `kind` on the `scripts` entry, nothing else.
- `api/src/services/`: decisions. A service interprets the request, calls repository functions by their domain names, maps entities to DTOs, and shapes the reply. It never imports `N/*`, `repositories/generated/context.gen` or the repository package; entity types from `common/types/models.gen` are fine, and the DTO types come from the controller's `endpoints.ts` with `import type` (types only, never the endpoints value).
...
- `api/src/lib/`: transport plumbing (`defineRestlet`, `defineSuitelet`, `endpoint`, `ApiError`) plus `suiteletClient.ts`, which calls another Suitelet controller of this application server-side: `createSuiteletClient<UserRolesEndpoints>(scripts.userRoles)`. Only a repository builds one.
...
- **Layers.** Endpoint calls service, service calls repository, repository composes specifications. `npm run lint` enforces the imports each layer may make (endpoints never query; services and repositories see a controller's `endpoints.ts` as types only; the client sees it as types only; models know nothing about the wire); a violation names the layer that should do the work instead.
