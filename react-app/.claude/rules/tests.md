...
- Each layer is tested against a fake of the layer below it: a repository test mocks the generated `dbContext` with a fake carrying the sets it reads, a service test mocks the repository module under the name the service imports it as, and a hook test replaces the generated client's function (`vi.spyOn(user.api, 'roles')`). A `lib/` function has no layer below it: its test calls it with plain values and mocks nothing.
