...
- A service decides. It takes plain arguments (an id, a filter, one object for a create's fields), calls repository functions by their domain names, and returns a type it declares itself: `RoleSummary`, a `Pick` of {{#if netsuiteRepository}}an entity type{{/if}}{{#unless netsuiteRepository}}a type a repository returns{{/unless}}, or a composition such as `ActiveUserRoles`.{{#if netsuiteRepository}} Entity types from `../types/models.gen` are fine.{{/if}}
...
- Which role a function needs is its script's concern: a function that reads a record the caller's role cannot works only in a script whose deployment runs as another role, and its comment says so{{#if userRolesExample}} (`userService.getRolesByEmployee`){{/if}}.
