...
| 3 | `npm run generate`: rewrites `client/src/api/index.gen.ts` with the controller's types and, unless `browser: false`, its client `<name>Api`, and `api/src/scripts.gen.ts` with its script; then a hook under `client/src/hooks/` calling the client, and a test | `useActiveUserRoles.ts`, `client/__tests__/userQuery.test.ts` |
