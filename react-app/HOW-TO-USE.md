...
| `nspModel`, `nspModelAllDecorators`, `nspModelLine`, `nspModelSubrecordClass`, `nspModelBase` | `api/src/models/<Record>.ts`: a record (a native type through `NetsuiteRecordType`, a custom record by its id string); the same with every decorator once and a comment on each, to trim down; a sublist line; a subrecord class; an abstract base |
...
Behind the controller: a service under `api/src/services/` (`<subject>Service.ts`, the `nspService` snippet) that decides, repository functions under `api/src/repositories/` (`<subject>Repository.ts`, the `nspRepository` snippet) that read and write, and for a new record type a model under `api/src/models/` (the `nspModel` snippet, or `nspModelAllDecorators` to start from every decorator and trim; then `npm run generate`) with its `<record>Specifications.ts` (the `nspSpecification` snippet). The service takes plain arguments and returns a type it declares (`RoleSummary`, produced by `buildRoleSummary`); the controller imports that type to build its response shape, and nothing in a service imports from a controller, so a second controller can call the same service and shape its own reply. The shipped `userRoles` chain (`userRolesController.ts`, `userRolesService.ts`, `employeeRolesRepository.ts`, `employeeRolesSpecifications.ts`, `api/src/models/EmployeeRole.ts`) is the reference. Each layer has a test snippet (`nspTestController`, `nspTestService`, `nspTestRepository`, `nspTestRepositorySuitelet`, `nspTestHook`): the file name picks the names, and the mocks are already in the shape the shipped tests use.
